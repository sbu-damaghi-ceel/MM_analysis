...
class MetaClusterGui():
    ...
    Args:
        metaclusterdata (MetaClusterData):
        ...
        width (float):
            Adjust the actual width to accommodate monitor size, resolution, zoom, etc.
        debug (bool):
        ...
        show_cluster_pixel_counts (bool):
            If True, display the pixel-count bar chart (ax_cp); if False, hide it.
    """

    def __init__(
        self,
        metaclusterdata,
        heatmapcolors=DEFAULT_HEATMAP,
        width=17.0,
        debug=False,
        enable_throttle=True,
        show_cluster_pixel_counts=True  # <--- NEW ARG
    ):
        ...
        # Make the bar chart optional
        self.show_cluster_pixel_counts = show_cluster_pixel_counts
    ...
    def make_gui(self):
        ...
        height_ratios = [
            6 * marker_ratio,
            self.mcd.marker_count * marker_ratio,
            marker_ratio,
            marker_ratio
        ]
        ...
        (self.fig, (
            ...
            (self.ax_03, self.ax_cl, self.ax_ml)
        )) = subplots
        ...
        # x-axis metacluster color labels
        ...
        # colorbar axis
        # We'll create the colorbar below, referencing self.im_c (cluster heatmap image).
        self.ax_cb.axis('off')

        # Make pixelcount bar chart optional
        self.rects_cp = []
        self.labels_cp = []

        if self.show_cluster_pixel_counts:
            ...
            # Hide the ax_cp axis entirely if we don't want pixel counts
            self.ax_cp.set_visible(False)
    ...
    def make_widgets(self):
        """Create the ipywidgets that display below the GUI plot."""
    ...
    def selection_mask(self):
        ...
        def is_selected(cluster):
            return 1 if (cluster in self.selected_clusters) else 0
    ...
    def update_gui(self):
        ...
        def _preplot(df):
            # compute zscore row-wise, then clip
        ...
        if np.allclose(zscore_cap % 1, 0) or np.allclose(zscore_cap % 1, 1):
            ...
            # fractional intervals in increments of 0.5
        ...
        # distinct_cmap: each unique integer gets a distinct color
        assert len(self.mcd.metaclusters.index) <= self.mcd.cluster_count, \
            "Can't support num metaclusters > cluster count"
        mc_cmap = distinct_cmap(self.mcd.cluster_count)  # re-use since #metaclusters <= #clusters
        ...
        # update cluster pixel-counts bar chart only if desired
        if self.show_cluster_pixel_counts and self.ax_cp is not None:
    ...
    def remap_current_selection(self, metacluster):
        """Instruct the MetaClusterData to remap the selected clusters to a given metacluster id."""
